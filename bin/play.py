...
if __name__ == "__main__":
    ...
    if window:
        ...
        time.sleep(.3) # wait for application to show up on screen 
        size = {
        ...
        with mss.MSS() as sct:
            while not is_finished: # main loop for screen analysis
                ...
                screenshot_grey = screenshot_grey[new_top : new_height, 200 : new_width]
        ...
        keyboard.release('s')
        print ("1-1 Completed :)")
                # if mario_coords is not None:
                #     cv.rectangle(screenshot_grey, mario_coords, (mario_coords[0] + mario_shape[0], 
                #                                                 mario_coords[1] + mario_shape[1]), (0,0,0), 2)
                
                # if len(goombas) != 0:
                #     for goomba in goombas:
                #         cv.rectangle(screenshot_grey, goomba, (goomba[0] + goomba_shape[0], 
                #                                                     goomba[1] + goomba_shape[1]), (0,0,0), 2)

                # plt.imshow (screenshot_grey, cmap="grey")
                # plt.show()
